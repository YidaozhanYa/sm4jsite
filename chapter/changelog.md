...
-修复了只发生在Android 10上的存储问题。

-修复了多人游戏时生命条显示不正确的问题。

-修复了在加载带有三连问号砖的关卡时出现的崩溃问题。

-选择空皮肤时不再崩溃。

-更正了噩梦模式下不能使用门或管道的错误。

-修正了Android系统中关卡选择器中缺少某些关卡的错误。
...
-优化模式实装。开启优化模式后，安卓系统的性能得到了一定的提升。
...
-材质包系统更新到V3，我们正在努力让它更快地加载纹理。

-现在，你可以加载以文件夹形式的材质包，不用压缩，使加载速度更快，避免一些错误。

（译者注：请务必在文件夹内放置一个名为“.nomedia”的文件夹，否则如果你是手机版，相册会爆炸）
...
-现在自动衔接的图块更细致了。 (感谢Snaker帮我做的)

-游戏现在可以识别字母 "N"。

-现在可以打出彩色字。(感谢JustinGdnz)
...
-增加了快速加载系统，使得重新启动关卡时的加载时间缩短。（可能会在安卓系统上造成问题）
...
（译者补充：-游戏的标题改为Super Mario 4 Jugadores Legacy，即超级马力欧 四个玩家 稳定版本。）

（译者补充：-现在电脑版游戏的数据文件夹位置更改，从%AppData%\Super_Mario_4_Jugadores 改为 %AppData%\SM4JLegacy。）

这些都是这个版本的改动，我知道这些改动不多，我知道距离上一个版本过去了很多时间，但是考虑到很多内容已经丢失，无法恢复，所以请谅解。
